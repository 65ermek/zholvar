...
class SenderListTable extends Table
{
    /**
     * Data source.
     *
    ...
    protected function columns(): array
    {
        ...
        return [
            ...
            TD::make('action', 'Действия')
                ->width('130px')
                ->render(function (User $senders){
                    return Group::make(
                        [
                            ModalToggle::make()
                                ->icon('pencil')
                                ->type(Color::SUCCESS())
                                ->modal('editSender')->method('update')
                                ->modalTitle('Редактирование отравителя'.'  '.$senders->name)
                                ->asyncParameters([
                                    'sender' => $senders->id
                                ]),
